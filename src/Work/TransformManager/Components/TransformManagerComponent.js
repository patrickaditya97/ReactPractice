...
class TransformManagerComponent extends Component {
    constructor(props) {
        ...
        this.getSelectionBoxBounds     = this.getSelectionBoxBounds.bind(this);
        this.getObjectsBounds          = this.getObjectsBounds.bind(this);
        this.getSelectionBox           = this.getSelectionBox.bind(this);
        this.isTextOnly                = this.isTextOnly.bind(this);
        this.updateInitialSelectionBox = this.updateInitialSelectionBox.bind(this);
        this.getMousePosition          = this.getMousePosition.bind(this);
        this.setCursor                 = this.setCursor.bind(this);
        this.resetCursor               = this.resetCursor.bind(this);
        this.isVerticalScale           = this.isVerticalScale.bind(this);
        this.isHorizontalScale         = this.isHorizontalScale.bind(this);
        ...
        this.updateResizeValues        = this.updateResizeValues.bind(this);
        this.getSelectionSnapPoints    = this.getSelectionSnapPoints.bind(this);
                ...
                if (item.get("type") === "GROUP" && item.get("subType") === "TXTGRP" && !item.get("isOverlay")) {
                    if (item.get("isExpands")) {
                        ...
                        var svgItem = this.props.selectedChildren.filter(function (obj, key) { 
                            return item.get("groupChildren").indexOf(key) !== -1 }).find(x => x.get("type") === "TEXT");
    ...
    resizeSelection(e) {
        ...
        this.selectedItems.entrySeq().forEach(([key, item]) => {
            ...
            if (item.get('type') === 'FRAME' && item.get('imgDetails') !== undefined) {
                // let current_item_img = current_item.getElementsByClassName('img_id_3')[0];
                // let frameX = item_width * item.getIn(['imgDetails', 'xRatio'])
                // let frameY = item_height * item.getIn(['imgDetails', 'yRatio'])
        
                // current_item_img.style.transform = `translate(${frameX}px, ${frameY}px) rotateZ(${item.get("angle")}deg) ${flip}`;
            }
    ...
    initiateMove(e) {
        ...
        if ((e.timeStamp - this.lastDown) < 250 && !this.isRightMouse(e)) {
            if (this.props.selectedItems.size === 1) {
                ...
                else if (!this.props.isFraming && this.selectedItems.get(this.props.selectedItems.get(0)).get("type") === "FRAME")
    ...
    render() {
        ...
        transformManager = <div data-html2canvas-ignore="true" key={this.props.keyframe} className={selectionBoxClass} ref={(instance) => this.selectionBoxElement = instance} data-id={id} style={{...selectionBoxStyles}} >
        ...
        {/*
          ...
          {(this.isTextOnly() && this.props.textStatus.get("isFocused")) ? <span className="text-inner-box" data-id={id} onMouseDown={this.initiateMove} onMouseEnter={(e) => this.setCursor(e, "move")} onMouseLeave={(e) => this.resetCursor(e)} style={{ top: "-15px", left: "-15px", right: "-15px", height: "15px", position: "absolute" }}></span> : null}
          {(this.isTextOnly() && this.props.textStatus.get("isFocused")) ? <span className="text-inner-box" data-id={id} onMouseDown={this.initiateMove} onMouseEnter={(e) => this.setCursor(e, "move")} onMouseLeave={(e) => this.resetCursor(e)} style={{ top: "-15px", right: "-15px", bottom: "-15px", width: "15px", position: "absolute" }}></span> : null}
          {(this.isTextOnly() && this.props.textStatus.get("isFocused")) ? <span className="text-inner-box" data-id={id} onMouseDown={this.initiateMove} onMouseEnter={(e) => this.setCursor(e, "move")} onMouseLeave={(e) => this.resetCursor(e)} style={{ left: "-15px", right: "-15px", bottom: "-15px", height: "15px", position: "absolute" }}></span> : null}
          {(this.isTextOnly() && this.props.textStatus.get("isFocused")) ? <span className="text-inner-box" data-id={id} onMouseDown={this.initiateMove} onMouseEnter={(e) => this.setCursor(e, "move")} onMouseLeave={(e) => this.resetCursor(e)} style={{ left: "-15px", top: "-15px", bottom: "-15px", width: "15px", position: "absolute" }}></span> : null}
